\newcommand{\thesistitle}{Organization of Retinal Ganglion Cell Axons in the Developing Mouse Retinogeniculate Pathway}
\newcommand{\thesisauthor}{Austen Sitko}
\newcommand{\thesisyear}{2016}

%%%
%%% Packages
%%%
\usepackage[pdftex]{graphicx}

\usepackage{afterpage}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{amsthm}
\usepackage{array}
\usepackage[english]{babel}
\usepackage[font=small,margin=0.5in]{caption}
\usepackage{ccicons}
\usepackage{color}
\usepackage{enumitem}
\usepackage{fancyhdr}
\usepackage[T1]{fontenc}
\usepackage{float}
\usepackage{geometry}
\usepackage{graphics}
\usepackage{makeidx}
\usepackage{mathtools}
\usepackage{multirow}
\usepackage{jneurosci}
\usepackage{pdflscape}
\usepackage{relsize}
\usepackage[section]{placeins}
\usepackage{sidecap}
\usepackage{subcaption}
\usepackage{subfloat}
\usepackage{textcomp}


\setcounter{secnumdepth}{4}

\newcommand{\textapprox}{\raisebox{0.5ex}{\texttildelow}}
\newcommand{\superscript}[1]{\ensuremath{^{\textrm{#1}}}}
\newcommand{\subscript}[1]{\ensuremath{_{\textrm{#1}}}}
\newcommand{\ddg}{\ensuremath{{\Delta}{\Delta}G}}
\newcommand{\plusorminus}{\ensuremath{\pm}}

\newcommand{\invivo}{{\it in vivo}}
\newcommand{\Invivo}{{\it In vivo}}
\newcommand{\invitro}{{\it in vitro}}
\newcommand{\Invitro}{{\it In vitro}}
\newcommand{\exvivo}{{\it ex vivo}}
\newcommand{\inutero}{{\it in utero}}

%
% We use the hyperref package and customize it for optimal PDF
%dvipdfm,
\usepackage[pdftitle={\thesistitle},pdfauthor={\thesisauthor},pdfpagemode={UseOutlines},bookmarks,bookmarksopen=true,pdfstartview=,bookmarksnumbered=true,hidelinks]{hyperref}

%%%
%%% Margins
%%%
\paperwidth=8.5in
\paperheight=11in

% 1in + hoffset + oddsidemargin + textwidth + marginparsep + marginparwidth
% For PhD at Columbia we have single side theses and 1.5in left margin
% The settings below leave 1.5 inch margin at the left and 1 inch at the right
% for US Letter paper
\setlength{\hoffset}{0.0in}
\setlength{\oddsidemargin}{0.0in}
% change me to 0.5 for binding
\setlength{\textwidth}{6.5in}
\setlength{\evensidemargin}{0mm}

% 1in + voffset + topmargin + headheight + headsep + textheight + footskip
% For PhD thesis we also need an extra inch at the bottom
% 1inch = 72 pt
\setlength{\voffset}{0.0in}
\setlength{\topmargin}{.0in}
\setlength{\headheight}{14pt}
\setlength{\headsep}{22pt}
\setlength{\textheight}{8.5in}
\setlength{\footskip}{0pt}

%%%
%%% Spacing
%%%
\newcommand{\singlespace}{\renewcommand{\baselinestretch}{1.15} \small \normalsize}
\newcommand{\oneandhalfspace}{\renewcommand{\baselinestretch}{1.3} \small \normalsize}
\newcommand{\doublespace}{\renewcommand{\baselinestretch}{1.5} \small \normalsize}
\newcommand{\normalspace}{\doublespace}
\footnotesep=1\baselineskip

%%%
%%% Counters depth
%%%
\setcounter{secnumdepth}{4}
\setcounter{tocdepth}{4}

\newcommand{\thesistitlepagewithschedule}{
    \normalspace
    \thispagestyle{empty}
    \begin{center}
        \textbf{\LARGE \thesistitle}
        \\[1cm]
        \textbf{\LARGE \thesisauthor}
        \\[4cm]
        12:00 -- 3:00 PM\\
        Monday, December 12, 2016\\
        Physicians \& Surgeons Building\\
        630 West 168th Street\\
        Room 14-240 \\[7cm]
        \textbf{\Large Columbia University} \\[5mm]
        \thesisyear
    \end{center}
    \clearpage
}



%%%
%%% Title page.
%%%
\newcommand{\thesistitlepage}{
    \normalspace
    \thispagestyle{empty}
    \begin{center}
        \textbf{\LARGE \thesistitle} \\[1cm]
        \textbf{\LARGE \thesisauthor} \\[8cm]
        Submitted in partial fulfillment of the \\
        requirements for the degree \\
        of Doctor of Philosophy \\
        in the Graduate School of Arts and Sciences \\[4cm]
        \textbf{\Large Columbia University} \\[5mm]
        \thesisyear
    \end{center}
    \clearpage
}

%%%
%%% Copyright page.
%%%
\newcommand{\thesiscopyrightpage}{
    \thispagestyle{empty}
    \strut \vfill
    \begin{center}
    \ccbysa \thesisyear \\
    \thesisauthor \\
    This work is licensed under the Creative Commons Attribution-ShareAlike 3.0 Unported (CC BY-SA 3.0) license.\\
    Please read more about what this implies at http://creativecommons.org/licenses/by-sa/3.0/.\\
    The \LaTeX files used to create this document are available at https://github.com/asitko/columbia\_thesis.
    \end{center}
    \cleardoublepage
}

%%%
%%% Abstract page.
%%%
\newcommand{\thesisabstract}{
    \thispagestyle{empty}
    \begin{center}
    \textbf{\LARGE Abstract} \\[1cm]
     %\textbf{\LARGE \thesistitle} \\[1cm]
     %\textbf{\LARGE \thesisauthor} \\[1cm]
    \end{center}
    \input{special_sections/abstract}
    \cleardoublepage
}

%%%
%%% Miscellaneous
%%%
\newcommand{\draft}{
    \renewcommand{\normalspace}{\singlespace}
    \normalspace
    \chapter*{Draft. Version \today}
\clearpage }

